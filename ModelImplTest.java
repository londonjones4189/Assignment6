...
public class ModelImplTest {
  ...
  public void testExamineGainLoss0DayDiff() {
    assertEquals("Stock stayed the same \uD83D\uDFF0, value is $194.03",
  ...
  public void testExamineGainLoss0DayDiffAMZN() {
    assertEquals("Stock stayed the same \uD83D\uDFF0, value is $178.34",
            model.examineGainLossDate(
  ...
  public void testExamineGainLossSameDay1DayDiff() {
    assertEquals("Stock gained value, stock value increased by $0.96 \uD83D\uDCC8",
  ...
  public void testExamineGainLossSameDay1DayDiffAMZN() {
    assertEquals("Stock lost value, stock decreased by $-2.88 📉",
  ...
  public void testExamineGainLoss7DayDiff() {
    assertEquals("Stock lost value, stock decreased by $-10.71 \uD83D\uDCC9",
            model.examineGainLossDate("AAPL",
  ...
  public void testExamineGainLoss7DayDiffAMZN() {
    assertEquals("Stock lost value, stock decreased by $-8.90 \uD83D\uDCC9",
            model.examineGainLossDate("AMZN",
                    LocalDate.of(2024, 04, 12),
                    LocalDate.of(2024, 04, 22)));
  }
  ...
  public void testExamineGainLossSameDay10DayDiff() {
    assertEquals("Stock gained value, stock value increased by $2.99 \uD83D\uDCC8",
            model.examineGainLossDate("AAPL",
  ...
  public void testExamineGainLossSameDay10DayDiffAMZN() {
    assertEquals("Stock lost value, stock decreased by $-5.20 \uD83D\uDCC9",
            model.examineGainLossDate("AMZN",
                    LocalDate.of(2024, 05, 20),
                    LocalDate.of(2024, 06, 03)));
  }
  ...
  public void testExamineGainLoss100DayDiff() {
    assertEquals("Stock gained value, stock value increased by $7.84 \uD83D\uDCC8",
            model.examineGainLossDate("AAPL",
  ...
  public void testExamineGainLossSameDay100DayDiffAMZN() {
    assertEquals("Stock gained value, stock value increased by $24.61 \uD83D\uDCC8",
            model.examineGainLossDate("AMZN",
                    LocalDate.of(2024, 01, 10),
                    LocalDate.of(2024, 06, 03)));
  }
  ...
  public void testExamineGainLossNetGainLargeRange() {
    assertEquals("Stock gained value, stock value increased by $158.67 \uD83D\uDCC8",
            model.examineGainLossDate("AAPL",
  ...
  public void testExamineGainLossNetGainLargeRangeAMZN() {
    assertEquals("Stock gained value, stock value increased by $143.61 \uD83D\uDCC8",
            model.examineGainLossDate("AMZN",
                    LocalDate.of(2005, 05, 17),
                    LocalDate.of(2024, 06, 03)));
  }
  ...
  public void testExamineGainLossNetLoss() {
    assertEquals("Stock lost value, stock decreased by $-25.38 \uD83D\uDCC9",
            model.examineGainLossDate("AAPL",
  ...
  public void testXDayAvgCrossOver30Day() throws IOException {
    ...
    assertEquals("169.48", model.examXAvgOverDays("GOOG", resultsDate.get(0),
            30));
    assertEquals("Stock stayed the same \uD83D\uDFF0, value is $177.4",
  ...
  public void testCreatePortfolio() throws IOException, ParserConfigurationException,
    ...
    model.createPortfolio("Portfolio 1");
    portfolioList.add("Portfolio 1");
    Assert.assertEquals(portfolioList, model.getPortfolioList());
    model.createPortfolio("Portfolio 2");
    portfolioList.add(0, "Portfolio 2");
    Assert.assertEquals(portfolioList, model.getPortfolioList());
  }
  ...
  public void testgetValue() throws IOException, ParserConfigurationException,
          TransformerException {
    model.createPortfolio("Portfolio 1");
    assertEquals("$0.00", model.getValue("Portfolio 1", date));
    model.addStock("AAPL", 13, "Portfolio 1");
    model.createPortfolio("Portfolio 2");
    //two portfolio with same stock at different values
    model.addStock("ABBV", 200, "Portfolio 2");
    assertEquals("$31262.00", model.getValue("Portfolio 2", date2));
    model.createPortfolio("Portfolio 3");
    model.addStock("ABBV", 150, "Portfolio 3");
    assertEquals("$23446.50", model.getValue("Portfolio 3", date2));
    //two portfolios with same stock same value
    model.createPortfolio("Portfolio 4");
    model.addStock("ABBV", 200, "Portfolio 4");
    assertEquals("$31262.00", model.getValue("Portfolio 4", date2));
  }

  @Test(expected = IllegalArgumentException.class)
  public void testDateWeekend() {
    model.getValue("Portfolio 2",
            LocalDate.of(2024, 05, 18));
  }

  @Test(expected = IllegalArgumentException.class)
  public void testPortfolioNotFound() {
    model.getValue("Portfolio 2", date);
  }
  ...
  public void testAddStock() throws ParserConfigurationException, IOException,
    ...
    assertEquals("13 shares from AAPL added to Portfolio 1",
            model.addStock("AAPL", 13,
            "Portfolio 1"));
    //returns correct value after 1 stock added
    assertEquals("$2499.25", model.getValue("Portfolio 1", date));
    //adds onto a stock that is already in the folder
    assertEquals("13 shares from AAPL added to Portfolio 1",
            model.addStock("AAPL", 13,
            "Portfolio 1"));
    //adds onto the stock correctly
    assertEquals("$4998.50", model.getValue("Portfolio 1", date));
    //assertEquals("A added to Portfolio 1", model.addStock("A", 0,
    //  "Portfolio 1"));
    assertEquals("$4998.50", model.getValue("Portfolio 1", date));
    //still the same even with other portfolio
    assertEquals("$0.00", model.getValue("Portfolio 2", date));
    //add stock to this portilo and other portfolio unaffected
    assertEquals("10 shares from A added to Portfolio 2",
            model.addStock("A", 10,
            "Portfolio 2"));
    //coreect value of portfolio 2
    assertEquals("$1304.10", model.getValue("Portfolio 2", date));
    //portfolio 1 stays unaffected
    assertEquals("$4998.50", model.getValue("Portfolio 1", date));
    assertEquals("$0.00", model.getValue("Portfolio 3", date));
    assertEquals("$0.00", model.getValue("Portfolio 3", date));
    assertEquals("10 shares from A added to Portfolio 3",
            model.addStock("A", 10,
            "Portfolio 3"));
    assertEquals("$1304.10", model.getValue("Portfolio 3", date));
  }
  ...
  public void testRemoveStockSome() throws ParserConfigurationException, IOException,
    ...
    model.addStock("AAPL", 13, "Portfolio 2");
    model.addStock("A", 2, "Portfolio 2");
    //added to portfolio 2
    model.addStock("AAPL", 13, "Portfolio 1");
    model.addStock("A", 2, "Portfolio 1");
    assertEquals("$2760.07", model.getValue("Portfolio 1", date));
    //remove from portfolio 2
    model.removeStock("AAPL", 13, "Portfolio 2");
    assertEquals("$260.82", model.getValue("Portfolio 2", date));
    model.removeStock("A", 1, "Portfolio 2");
    assertEquals("$130.41", model.getValue("Portfolio 2", date));
  }
  ...
  public void testNotCorrectStock() throws ParserConfigurationException, IOException,
          TransformerException {
    model.createPortfolio("Portfolio 1");
    model.addStock("TSLA", 10, "Portfolio  5");
  }
  ...
  public void testNotCorrectPortfolio() throws ParserConfigurationException, IOException,
          TransformerException {
    model.createPortfolio("Portfolio 1");
    model.addStock("APPL", 10, "Portfolio  5");
  }
  ...
  public void testNotCorrectTickerRemove() throws ParserConfigurationException, IOException,
          TransformerException {
    model.createPortfolio("Portfolio 1");
    model.addStock("AMZN", 10, "Portfolio 1");
    model.removeStock("AAPL", 10, "Portfolio 1");
  }
  ...
  public void testNotCorrectPortoflioRemove() throws ParserConfigurationException, IOException,
          TransformerException {
    model.createPortfolio("Portfolio 1");
    model.addStock("APPL", 10, "Portfolio 1");
    model.removeStock("AAPL", 5, "Portfolio 2");
  }
  ...
  public void testGetPortfolioList() throws ParserConfigurationException, IOException,
    ...
    model.createPortfolio("Portfolio 2");
    model.createPortfolio("Portfolio 3");
    List<String> expectedPortfolios = new ArrayList<>(Arrays.asList("Portfolio 1",
  ...
  public void testPortfolioInvalidDate() throws ParserConfigurationException, IOException,
          TransformerException {
    model.createPortfolio("Portfolio 1");
    model.addStock("AAPL", 10, "Portfolio 1");
    model.getValue("Portfolio 1", LocalDate.of(1990, 05, 18));
  }
  ...
  public void testPortfolioDateMissing() throws ParserConfigurationException, IOException,
    ...
    model.createPortfolio("Portfolio 1");
    model.addStock("AAPL", 10, "Portfolio 1");
    model.getValue("Portfolio 1", LocalDate.of(2024, 05, 18));
  }
}