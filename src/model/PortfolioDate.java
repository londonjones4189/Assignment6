...
public class PortfolioDate implements IPortfolioMax {
  ...
  public IPortfolioMax removeStockNew(LocalDate date, Stock stock, double shares) {
    ...
    // Return a new model.PortfolioDate with the updated transaction log
  ...
  public void generatePortfolioXML() {
    try {
      ...
      // root element
      Element rootElement = doc.createElement("model.Portfolio");
  ...
  @Override
  public List<String> getDatesInLog() {
    List<String> datesInLog = new ArrayList<>();
    for (Map.Entry<LocalDate, List<Transaction>> entry : this.transLog.entrySet()) {
      LocalDate date = entry.getKey();
      datesInLog.add(date.toString());
    }
